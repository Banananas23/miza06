...
public class Soba{
	...
	public boolean odpriOkna(){
		if(oknaOdprta){
			...
			if(stOken == 0){
				System.out.println("Soba nima oken, zato jih ne morem odpreti");
				return false;
			}
	 ...
	 */
	public boolean prizgiGretje(){
		if(ogrevanjeVkljuceno){
			return false;
		}
		
		else if(oknaOdprta){
			System.out.println("Potrebno je zapreti okna ... zapiram okna.");
			oknaOdprta = false;
			...
			if(temperatura>27){
				System.out.println("Dosežena je bila zgornja stopnja temperature " + temperatura + ", zato se je ogrevanje ugasnilo");
				return false;
			}
